...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == "add_candidate") {
    ...
    try {
        ...
        if (!$checkRegStmt) throw new Exception("Prepare failed: " . $conn->error);
        $checkRegStmt->bind_param("ii", $userId, $electionId);
        $checkRegStmt->execute();
        $checkRegResult = $checkRegStmt->get_result();
        ...
        if (!$checkPostStmt) throw new Exception("Prepare failed: " . $conn->error);
        $checkPostStmt->bind_param("si", $position, $electionId);
        ...
        $stmt->bind_param("iiiss", $userId, $electionId, $position, $bio, $profilePhoto);
        if ($stmt->execute()) {
    ...
    if ($_POST['action'] == "delete_candidate") {
        ...
        if ($candidateId > 0) {
            try {
                ...
                $stmt_select->bind_param("i", $candidateId);
                $stmt_select->execute();
                $stmt_select->bind_result($profile_photo);
                $stmt_select->fetch();
                $stmt_select->close();
                ...
                if (!empty($profile_photo)) {
                    ...
                    if ($file_path && file_exists($file_path)) {
                        if (!unlink($file_path)) {
                            throw new Exception("Error deleting profile photo file.");
                        }
                    }
                }
                ...
                $stmt_delete->bind_param("i", $candidateId);
                if ($stmt_delete->execute()) {
                    echo json_encode(["status" => "success"]);
                } else {
                    throw new Exception("Error deleting candidate: " . $stmt_delete->error);
                }
                $stmt_delete->close();
            } catch (Exception $e) {
...
// Fetch Candidates
try {
    $candidatesStmt = $conn->prepare("SELECT c.*, e.title AS election_title, u.username FROM contesters c JOIN elections e ON c.election_id = e.id JOIN users u ON c.user_id = u.id");
    if (!$candidatesStmt) throw new Exception("Prepare failed: " . $conn->error);
...
<head>
    ...
    <style>
        ...
        .form-container button { background: #3498db; color: white; padding: 10px; border: none; cursor: pointer; border-radius: 4px; }
            ...
            <form id="add-candidate-form" enctype="multipart/form-data" method="POST">
                <select name="election_id" id="election-select" required onchange="this.form.submit();">
                    ...
                    <?php foreach ($elections as $election): ?>
                        <option value="<?php echo $election['id']; ?>" <?php if ($selectedElectionId == $election['id']) echo 'selected'; ?>> <?php echo htmlspecialchars($election['title']); ?> </option>
                    <?php endforeach; ?>
                </select>
                ...
                <select name="position" id="position-select" required>
                    ...
                    <?php foreach ($positionsForSelectedElection as $position): ?>
                        <option value="<?php echo htmlspecialchars($position['postname']); ?>"> <?php echo htmlspecialchars($position['postname']); ?> </option>
                    <?php endforeach; ?>
                </select>
            ...
            <tbody id="candidate-list">
                <?php foreach ($candidates as $candidate): ?>
                    <tr id="candidate-<?php echo $candidate['id']; ?>">
                        ...
                        <td><img src="../faces/<?php echo htmlspecialchars($candidate['profile_photo']); ?>" alt="Profile Photo" class="profile-img"></td>
                        ...
                        <td><?php echo htmlspecialchars($candidate['bio']); ?></td>
...
$(document).ready(function() {
    ...
    $(document).on("click", ".delete-btn", function() {
        ...
        $.post("manage_candidates.php", { action: "delete_candidate", delete_id: candidateId }, function(response) {
            if (response.status === "success") {
                $("#candidate-" + candidateId).fadeOut(500, function() { $(this).remove(); });
            } else {
    ...
    $(document).ready(function() {
        $('#election-select').change(function() {
            let electionId = $(this).val();
            if (electionId) {
                $.ajax({
                    url: 'fetch_candidates_and_positions.php',
                    type: 'POST',
                    data: { election_id: electionId },
                    dataType: 'json',
                    success: function(response) {
                        $('#candidate-select').html('<option value="">Select Candidate</option>');
                        $('#position-select').html('<option value="">Select Position</option>');
                        
                        $.each(response.candidates, function(index, candidate) {
                            $('#candidate-select').append('<option value="' + candidate.id + '">' + candidate.name + '</option>');
                        });
                        
                        $.each(response.positions, function(index, position) {
                            $('#position-select').append('<option value="' + position.name + '">' + position.name + '</option>');
                        });
                    }
                });
            } else {
                $('#candidate-select, #position-select').html('<option value="">Select</option>');
            }
        });
    });
</script>
